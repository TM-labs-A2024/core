...
const deleteHealthRecordDataByID = `-- name: DeleteHealthRecordDataByID :exec
UPDATE health_record
SET public_key = null
WHERE id = $1
`

func (q *Queries) DeleteHealthRecordDataByID(ctx context.Context, id pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteHealthRecordDataByID, id)
	return err
}
...
const getGovernmentEnrollmentRequestByInsitutionID = `-- name: GetGovernmentEnrollmentRequestByInsitutionID :one
SELECT created_at, updated_at, id, institution_id, government_id, pending, approved
FROM government_enrollment_request
WHERE institution_id = $1
`

func (q *Queries) GetGovernmentEnrollmentRequestByInsitutionID(ctx context.Context, institutionID pgtype.UUID) (GovernmentEnrollmentRequest, error) {
	row := q.db.QueryRow(ctx, getGovernmentEnrollmentRequestByInsitutionID, institutionID)
	var i GovernmentEnrollmentRequest
	err := row.Scan(
		&i.CreatedAt,
		&i.UpdatedAt,
...
const listPatientsByInstitutionID = `-- name: ListPatientsByInstitutionID :many
...
func (q *Queries) ListPatientsByInstitutionID(ctx context.Context, institutionID pgtype.UUID) ([]Patient, error) {
	rows, err := q.db.Query(ctx, listPatientsByInstitutionID, institutionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
