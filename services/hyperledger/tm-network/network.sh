...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating Main Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-main.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating Aux Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-aux.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "cfssl" ]; then
    ...
    peer_cert peer peer0.main.tmlabs.com main
    peer_cert admin Admin@main.tmlabs.com main

    infoln "Creating Aux Identities"
    #function_name cert-type   CN   org
    peer_cert peer peer0.aux.tmlabs.com aux
    peer_cert admin Admin@aux.tmlabs.com aux

    infoln "Creating Orderer Org Identities"
    #function_name cert-type   CN   
    orderer_cert orderer orderer.tmlabs.com
    orderer_cert admin Admin@tmlabs.com
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/main/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for Main and Aux"
...
# call the script to create the channel, join the peers of main and aux,
...
function networkDown() {
  ...
  # stop org3 containers also in addition to main and aux, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.tmlabs.com docker_peer0.main.tmlabs.com docker_peer0.aux.tmlabs.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/main/msp organizations/fabric-ca/main/tls-cert.pem organizations/fabric-ca/main/ca-cert.pem organizations/fabric-ca/main/IssuerPublicKey organizations/fabric-ca/main/IssuerRevocationPublicKey organizations/fabric-ca/main/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/aux/msp organizations/fabric-ca/aux/tls-cert.pem organizations/fabric-ca/aux/ca-cert.pem organizations/fabric-ca/aux/IssuerPublicKey organizations/fabric-ca/aux/IssuerRevocationPublicKey organizations/fabric-ca/aux/fabric-ca-server.db'
